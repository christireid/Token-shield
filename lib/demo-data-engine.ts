/**
 * Demo data engine — pure functions for generating and updating
 * dashboard demo data. Extracted from dashboard-provider.tsx for
...
export const MODULE_KEYS = ["guard", "cache", "context", "router", "prefix"] as const

export const EVENT_TYPES: DashboardEvent["type"][] = [
  "cache:hit",
  "cache:miss",
  "request:blocked",
  "router:downgraded",
...
export const INITIAL_USERS: UserBudget[] = [
  {
    userId: "usr_alice",
...
export function generateInitialPipelineMetrics(): PipelineStageMetric[] {
...
export function generateInitialProviderHealth(): ProviderHealthRecord[] {
...
export function generateInitialAnomalies(nextId: () => number): AnomalyRecord[] {
  const now = Date.now()
...
export function generateInitialAlerts(nextId: () => number): DashboardAlert[] {
  const now = Date.now()
  return [
    {
      id: nextId(),
...
export const INITIAL_DELTAS: DashboardData["kpiDeltas"] = {
...
/* ------------------------------------------------------------------ */
/*  Deep-clone initial users to avoid reference sharing                */
/* ------------------------------------------------------------------ */

export function cloneInitialUsers(): UserBudget[] {
...
export function createEmptyState(): DashboardData {
  return {
    totalSpent: 0,
    totalSaved: 0,
    savingsRate: 0,
...
/* ------------------------------------------------------------------ */
/*  Seed data generation (60 minutes of history)                       */
/* ------------------------------------------------------------------ */

export function generateSeedData(nextId: () => number): DashboardData {
  const now = Date.now()
  const points: DashboardData["timeSeries"] = []
  let cumSpent = 0
  let cumSaved = 0
  const modules: DashboardData["byModule"] = {
    guard: 0,
    cache: 0,
    context: 0,
    router: 0,
  ...
  const midBlocked = Math.round(blocked * 0.45)
  const midLatency = rand(130, 300)
  ...
  return {
    ...
    users: updatedUsers,
    anomalies: generateInitialAnomalies(nextId),
    pipelineMetrics: generateInitialPipelineMetrics(),
    providerHealth: generateInitialProviderHealth(),
    alerts: generateInitialAlerts(nextId),
  }
}

/* ------------------------------------------------------------------ */
/*  Pre-computed tick IDs for concurrent-safe interval updates          */
/* ------------------------------------------------------------------ */

export interface TickIds {
  eventId: number
  shouldGenerateAnomaly: boolean
  anomalyId: number
...
export function computeNextTick(prev: DashboardData, ids: TickIds): DashboardData {
  ...
  if (ids.shouldGenerateAlert) {
    ...
    newAlerts = [
      ...prev.alerts,
      { ...template, id: ids.alertId, timestamp: now, dismissed: false },
    ].slice(-10)
  }
