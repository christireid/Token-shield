  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: "string",
          description: "The search query string to find matching records in the database",
        },
...
describe("countToolTokens", () => {
  ...
  it("costOverRequests scales linearly with request count", () => {
    ...
    expect(result.costOverRequests(100)).toBeCloseTo(result.costPerRequest * 100, 10)
  ...
  it("costPerRequest uses the provided inputPricePerMillion", () => {
    ...
    expect(expensiveResult.costPerRequest).toBeCloseTo(cheapResult.costPerRequest * 100, 10)
  })
...
describe("optimizeToolDefinitions", () => {
  ...
  it("does not truncate descriptions that are <= 100 chars", () => {
    ...
    expect(result.optimized[0].function.description).toBe(sampleTool.function.description)
  })
  ...
  it("removes parameter descriptions that repeat the parameter name", () => {
    ...
    const optimizedProps = result.optimized[0].function.parameters!.properties!
    ...
    expect(optimizedProps["email"].description).toBe("The email address for sending notifications")
  })
  ...
  it("removes descriptions shorter than 10 characters", () => {
    ...
    expect(result.optimized[0].function.parameters!.properties!["id"].description).toBeUndefined()
  })
  ...
  it("reports savedTokens > 0 when optimizations apply", () => {
    ...
    expect(result.savedTokens).toBe(result.originalTokens - result.optimizedTokens)
  })
  ...
  it("provides suggestions array with details", () => {
    ...
    expect(result.suggestions.some((s) => s.includes("search_database"))).toBe(true)
  })
})
...
describe("predictOutputTokens", () => {
  ...
  it("matches code-generation pattern", () => {
    const result = predictOutputTokens("Write a function that sorts an array")
    expect(result.taskType).toBe("code-generation")
    expect(result.confidence).toBe("medium")
  })

  it("matches analysis pattern", () => {
    const result = predictOutputTokens("Explain how photosynthesis works in detail")
