...
**TokenShield is different.** It is a **Middleware SDK** that runs *inside* your application.
...
const client = createOpenAIAdapter(
  shield,
  (p) => new OpenAI().chat.completions.create(p as any)
);
...
const client = createAnthropicAdapter(
  shield,
  (p) => new Anthropic().messages.create(p as any)
);
...
- **👤 User Budget Manager:** Enforce limits on *your* customers (e.g., "User A gets $5/mo").
