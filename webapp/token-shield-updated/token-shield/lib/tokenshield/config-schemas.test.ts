...
describe("validateConfig", () => {
  ...
  it("throws for cache.similarityThreshold = 1.5 (maxValue: 1)", () => {
    expect(() =>
      validateConfig({ cache: { similarityThreshold: 1.5 } })
    ).toThrow();
  });
  ...
  it("throws for breaker.action = 'invalid' (not in picklist)", () => {
    expect(() =>
      validateConfig({ breaker: { limits: {}, action: "invalid" } })
    ).toThrow();
  });
  ...
  it("throws for guard.maxRequestsPerMinute = 0 (minValue: 1)", () => {
    expect(() =>
      validateConfig({ guard: { maxRequestsPerMinute: 0 } })
    ).toThrow();
  });
  ...
  it("throws for prefix.provider = 'azure' (not in picklist)", () => {
    expect(() =>
      validateConfig({ prefix: { provider: "azure" } })
    ).toThrow();
  });
  ...
  it("throws for router.complexityThreshold > 100", () => {
    expect(() =>
      validateConfig({ router: { complexityThreshold: 101 } })
    ).toThrow();
  });
