...
import { countExactTokens, countChatTokens, type ChatMessage } from "@/lib/tokenshield/token-counter"
...
export function TokenizerTest() {
  ...
  const avgAccuracy = results.length > 0
    ...
    : 0
  const maxDiff = results.length > 0
    ? Math.max(...results.map((r) => r.difference))
    : 0
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Sends {TEST_PROMPTS.length} prompts to GPT-4o-mini (max_tokens=1 to minimize cost) and compares our client-side gpt-tokenizer count against OpenAI{"'"}s usage.prompt_tokens. This proves we count tokens the same way OpenAI does.
        </p>
      </div>

      <Button onClick={runTest} disabled={running} className="min-h-[44px] w-full sm:w-auto">
        {running ? `Testing ${results.length + 1}/${TEST_PROMPTS.length}...` : `Run Test (${TEST_PROMPTS.length} API calls, ~$0.001)`}
      </Button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-card">
                ...
                <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Client</th>
                <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">OpenAI</th>
            ...
            <tbody>
              {results.map((r, i) => (
                <tr key={i} className="border-b border-border last:border-0">
                  ...
                  <td className={`px-3 py-2 text-right font-mono text-xs font-bold ${r.difference === 0 ? "text-primary" : r.difference <= 2 ? "text-chart-3" : "text-destructive"}`}>
                    {r.difference === 0 ? "EXACT" : `+${r.difference}`}
                  </td>
                  <td className={`px-3 py-2 text-right font-mono text-xs font-bold ${r.accuracy >= 99 ? "text-primary" : "text-chart-3"}`}>
      ...
      {results.length === TEST_PROMPTS.length && (
        <div className="rounded-lg border-2 border-primary/50 bg-primary/5 p-4 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            gpt-tokenizer uses the same BPE encoding (o200k_base) that OpenAI uses server-side. Any difference is from chat message overhead formatting, which varies by a few tokens.
          </p>
        </div>
      )}
    </div>
