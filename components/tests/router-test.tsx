...
export function ModelRouterTest() {
  ...
  const [simpleResults, setSimpleResults] = useState<{
    expensive: ModelResult
    cheap: ModelResult
  } | null>(null)
  const [complexAnalysis, setComplexAnalysis] = useState<ReturnType<
    typeof analyzeComplexity
  > | null>(null)
  const [simpleAnalysis, setSimpleAnalysis] = useState<ReturnType<typeof analyzeComplexity> | null>(
    null,
  )
  ...
  async function runTest() {
    ...
    try {
      ...
      const expensiveCost = calculateRealCost(
        "gpt-4o",
        expensiveRes.usage.prompt_tokens ?? 0,
        expensiveRes.usage.completion_tokens ?? 0,
      )
      const cheapCost = calculateRealCost(
        "gpt-4o-mini",
        cheapRes.usage.prompt_tokens ?? 0,
        cheapRes.usage.completion_tokens ?? 0,
      )
  ...
  const savings = simpleResults
    ? {
        costDiff:
          simpleResults.expensive.totalCost.totalCost - simpleResults.cheap.totalCost.totalCost,
        percentSaved:
          ((simpleResults.expensive.totalCost.totalCost - simpleResults.cheap.totalCost.totalCost) /
            simpleResults.expensive.totalCost.totalCost) *
          100,
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Sends {'"'}What is the capital of France?{'"'} to both GPT-4o ($2.50/M) and GPT-4o-mini
          ($0.15/M). Proves both answer correctly while showing the 16x price difference. Also shows
          complexity analysis of a hard prompt vs easy prompt.
        </p>
      </div>
          ...
          {[simpleResults.expensive, simpleResults.cheap].map((result) => (
            ...
            >
              <div className="font-mono text-xs font-bold text-primary">
                {result.model} (returned: {result.modelReturned})
              </div>
              <div className="space-y-1.5 text-sm">
                ...
                <Row
                  label="Input cost"
                  value={`$${result.totalCost.inputCost.toFixed(8)}`}
                  highlight
                />
                <Row label="Output cost" value={`$${result.totalCost.outputCost.toFixed(8)}`} />
                <Row
                  label="Total cost"
                  value={`$${result.totalCost.totalCost.toFixed(8)}`}
                  highlight
                />
        ...
        <div className="rounded-lg border-2 border-primary/50 bg-primary/5 p-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <div className="font-mono font-bold text-foreground">
                {savings.inputPriceRatio.toFixed(1)}x cheaper
              </div>
            </div>
          </div>
          <p className="text-xs text-muted-foreground">
            Both models answered correctly. The router would send this to gpt-4o-mini, saving{" "}
            {savings.percentSaved.toFixed(0)}% with identical quality for trivial questions.
          </p>
        </div>
      )}
    </div>
